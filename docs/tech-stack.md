...
- **`minilm` (default)** — `all-MiniLM-L6-v2`: 384 dims, English, ~10–15 ms per embed, ~23 MB. Downloaded as FP32 and quantized to INT8 at build time — the same pipeline earlier releases used, so existing databases stay compatible. The smallest image and the safe default for English-only content.
- **`gte-multilingual-base`** — 768 dims, ~70 languages, 8192-token context, ~325 MB, Apache-2.0. Queries embed in ~12 ms — near-MiniLM speed, since most of its parameters are vocabulary embeddings that cost little for short inputs — while longer chunks take ~150 ms. Strong cross-lingual retrieval (a query in one language finds content stored in another) with no task prefixes.
